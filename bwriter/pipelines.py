...
class ProcessingPipeline:
    ...
    def __init__(self, input_directory: Path,
        ...
        Args:
            ...
            llm_correction (str, optional): OpenAI LLM used in the correction step. Defaults to "gpt-4o-mini".
            llm_merge (str, optional): OpenAI LLM used in the merge step. Defaults to "gpt-4o".
            llm_sorter (str, optional): OpenAI LLM used for sorting. Defaults to "gpt-4o-mini".
        """
    ...
    def process_files(self, save_files: bool = True, **kwargs) -> Dict[str, Union[str, List[str]]]:
        """Split and correct inptut files

        Args:
            save_files (bool, optional): Whether to save the corrected files. Defaults to True.
        ...
        if save_files:
            ...
            self.corrector.save_text(corrected_splits,
                                     self.corrected_directory
                                     )
        return corrected_splits

    def merge_similar_splits(self,
        ...
        Args:
            ...
            save_files (bool, optional): Whether to save the merged files. Defaults to True.
            **kwargs: Additional keyword arguments for merging.
        """
        # Get text clusters
        text_clusters = self.merger.get_clusters_from_text(text_splits,
                                                           **kwargs
                                                           )
        # Merge clusters
        ...
        if save_files:
            self.merger.save_text(merged_clusters,
                                               self.merged_directory
                                               )
    ...
    def order_clusters(self, text_clusters: Dict[int, str]) -> List[str]:
        ...
        Args:
            text_clusters (Dict[int, str]): Dictionary of text clusters to order.
        """
    ...
    def run(self, process_kwargs, merge_kwargs, save_files: bool = True) -> str:
        ...
        Args:
            process_kwargs (dict): Keyword arguments for the text processing step.
            merge_kwargs (dict): Keyword arguments for the merging step.
            save_files (bool, optional): Whether to save the processed files. Defaults to True.
        """
        corrected_splits = self.process_files(save_files, **process_kwargs)
        texts_to_merge = [text["text"] for text in corrected_splits]
        merged_clusters = self.merge_similar_splits(texts_to_merge,
                                                    save_files,
                                                    **merge_kwargs
                                                    )
